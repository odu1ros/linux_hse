...
# letting user choose if he wants to play the next game (generate a word if yes, exit if no)
function start_new_game {
	read -n1 -p "Start a new game? [Y/n] " state
	echo
	
	if [[ "$state" == "Y" || "$state" == "y" ]]; then
		# shuffle textfile and select first word
